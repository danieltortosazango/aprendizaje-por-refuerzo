...
class DQN(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.fc1 = nn.Linear(input_dim, 128)  # Cambiar el tamaño de la primera capa
        self.fc2 = nn.Linear(128, 64)         # Nueva capa intermedia
        self.fc3 = nn.Linear(64, 64)          # Nueva capa intermedia
        self.fc4 = nn.Linear(64, output_dim)  # Cambiar el tamaño de la última capa
...
exploration_episodes = 30  # Numero de episodios de exploración guiada
iteraciones_a_la_izquierda = 10 # Numero de iteraciones que va a la izquierda
...
for episode in range(numero_episodios + 1):
    ...
    if episode < exploration_episodes:
        ...
        while not done and not truncated:
            if izquierda < iteraciones_a_la_izquierda:
                action = 0 # Carrito a la izquierda
            else: 
                action = 2 # Carrito a la derecha
    ...
    # Mostrar el episodio como un video usando OpenCV
    if episode_frames:
        for frame in episode_frames:
            cv2.imshow('Episodio ' + str(episode), frame)
    ...
    if episode % 10 == 0 and episode != 0: #cada 10 episodios imprimimimos el tiempo promedio y su recompensa promedia
        recompensa_promedia = recompensa_total / 10 #como es la recompensa de 10 episodios hacemos la media
        array_recompensas.append(recompensa_promedia)
        print("Recompensa promedia en pasar 10 episodios: " + str(recompensa_promedia))
        recompensa_total = 0 #reseteamos la recompensa total

    #print("Episodio:", episode, "Recompensa:", total_reward)

# Guardar la red neuronal en un archivo
torch.save(agent.model.state_dict(), "dqn_model.pth")
print("Red neuronal guardada en el archivo 'dqn_model.pth'.")

# Cerrar el entorno
env.close()

with open("recompensas_mountaincar_dqn.txt", "w") as txt_file:
