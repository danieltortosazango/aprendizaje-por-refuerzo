...
for episodio in range(num_episodios + 1):
    estado = env.reset()[0]
    estado = tuple(np.round(estado, decimals=1))  # Redondear los valores del estado y convertirlo en una tupla
    total_recompensa = 0
    done = False
    truncated = False
    ...
    while not done and not truncated:
        ...
        siguiente_estado = tuple(np.round(siguiente_estado, decimals=1))  # Redondear los valores del siguiente estado y convertirlo en una tupla
...
with open("recompensas_cartpole_qlearning.txt", "w") as txt_file:
